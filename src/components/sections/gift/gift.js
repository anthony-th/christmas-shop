...
const cardCategory = (filter) => {
  ...
  cardsContainer.append(
    ...openCategory.map((item) => createCard(item, { subtitle: 'h2', title: 'h3' }))
  );
};
...
categories.forEach((category, index) => {
  ...
  const tabBtn = createElement(
    'button',
    ['tabs-btn', 'user-select-none', 'cursor-pointer'],
    category.name,
    { 'aria-label': `Open card ${category.name}` }
  );
  if (index === 0) {
